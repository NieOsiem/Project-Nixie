...
import { FALL_RATE, JITTER_CYCLE, SPLASH_RATE } from "./shaders/weather.js";
...
describe("weather clock", () => {
  ...
  it("wraps where every animated term is continuous across it", () => {
    // The wrap shifts the drop lattice by TIME_WRAP_S * FALL_RATE cells and the splash phase by
    // TIME_WRAP_S * SPLASH_RATE periods. Both have to land on whole numbers, and the first on a
    // whole number of jitter cycles, or a wrap reshuffles every drop on screen in one frame.
    expect(fromWhole((TIME_WRAP_S * FALL_RATE) / JITTER_CYCLE)).toBeLessThan(1e-9);

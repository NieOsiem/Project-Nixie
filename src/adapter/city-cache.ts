...
const SIGNATURE_KEYS = ["terrain", "roads", "districts", "generation", "architecture", "regeneration", "schemaVersion", "generatorVersion"] as const;
...
function assertCurrentCity(
  ...
  let problems: string[];
  try {
    problems = validateCityStateV5(currentRaw);
  } catch {
  ...
  const current = currentRaw as CityStateV5;
...
export async function publishCompletePlanCache(
  ...
  const cityProblems = validateCityStateV5(city);

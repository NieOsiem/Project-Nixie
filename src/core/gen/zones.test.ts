...
describe("lotRegions", () => {
  it("covers everything with one region when there are no zones", () => {
    const regions = lotRegions(DEFAULT_ZONE_PARAMS, [], BOUNDS, 25);
  ...
  it("omits the leftover when zones cover the bounds", () => {
    const regions = lotRegions(DEFAULT_ZONE_PARAMS, [zone("z1", BOUNDS, 3)], BOUNDS, 25);
    expect(regions).toHaveLength(1);
  ...
  it("anchors the unzoned grid to world zero, not to the bounds", () => {
    // Bounds move on every edit; anchoring there would reshuffle the whole city the
    // moment a road extends past the old edge.
    const near = lotRegions(DEFAULT_ZONE_PARAMS, [], { x: -400, y: 900, width: 100, height: 100 }, 25);
    const far = lotRegions(DEFAULT_ZONE_PARAMS, [], { x: 7777, y: -3333, width: 5000, height: 5000 }, 25);
    expect(near[0]!.options.originPx).toEqual({ x: 0, y: 0 });

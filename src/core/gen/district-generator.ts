...
import { ROUTE_CLASS_REGISTRY, type CitySourceV5, type DistrictSource } from "./city.js";
...
function vehicleRoadCount(source: CitySourceV5): number {
  return source.roads.edges.filter((edge) => ROUTE_CLASS_REGISTRY.get(edge.classId)?.vehicle).length;
}

export function districtGenerationAvailability(source: CitySourceV5): DistrictGenerationAvailability {
...
function enabledPool(source: CitySourceV5): DistrictTypeId[] {
...
function hubs(source: CitySourceV5, bounds: ReturnType<typeof ringBounds>): Vec2[] {
...
export function districtRegionContext(source: CitySourceV5, blocks: readonly DerivedBlock[], polygon: Ring): DistrictRegionContext {
  const mask = source.terrain.urbanFootprint ?? source.terrain.land;
  const maskBounds = ringBounds(mask);
...
export function generateInitialDistricts(source: CitySourceV5): DistrictSource[] {

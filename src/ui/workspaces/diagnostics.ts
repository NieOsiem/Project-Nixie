import { districtDiagnostics, retryDistrictPlan, retryGeneratedWalls } from "../../adapter/canvas.js";
...
export function diagnosticsTrayHTML(entries: readonly Record<string, unknown>[]): string {
  ...
  const items = entries.map((entry) => {
    ...
    const retry = entry.retry === "walls" || entry.subsystem === "walls"
      ? `<button type="button" data-action="diagnostics-retry-walls">Retry generated walls</button>`
      : entry.retry === "plan"
        ? `<button type="button" data-action="diagnostics-retry-plan">Retry district planning</button>`
        : "";
...
export function diagnosticsWorkspace(): WorkspaceModule {
  return {
    ...
    onAction(action: string, _target: HTMLElement, ctx: WorkspaceContext): void {
      ...
      if (action === "diagnostics-retry-plan") ctx.run("district plan retry", retryDistrictPlan());
    },
